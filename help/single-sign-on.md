...
Configurez l’application [!DNL Marketo Measure] dans votre fournisseur d’identité. Consultez la documentation externe répertoriée ci-dessous pour les procédures.
    ...
    a. Lorsque vous êtes invité à saisir l’URL d’authentification unique, l’URL du destinataire ou l’URL de destination, l’URL du service client d’assertion SAML (ACS), utilisez [https://apps.bizible.com/BizibleSAML2/ReceiveSSORequest](https://apps.bizible.com/BizibleSAML2/ReceiveSSORequest)
...
Basculer vers la SSO personnalisée dans l’application [!DNL Marketo Measure]
    ...
    b. Par défaut, votre type de connexion sera défini sur « Utilisateurs CRM ». 
    
    c. Définissez le Type de connexion sur « Connexion unique personnalisée » pour lancer le processus de configuration.

Renseigner les paramètres de connexion pour la configuration de votre fournisseur d’identité

    a. Votre fournisseur d’identité peut fournir un document .xml de métadonnées IdP qui extraira les champs de configuration requis. Chargez le contenu du document .xml ou renseignez les trois champs ci-dessous à partir des résultats obtenus lors du processus de configuration du fournisseur d’identité. **Vous n&#39;avez pas besoin de compléter les deux.**
    
    i. URL de l’IdP : URL  [!DNL Marketo Measure]  doit pointer pour authentifier vos utilisateurs dans l’application  [!DNL Marketo Measure] . Parfois appelée « URL de redirection »
    ii. IdP émetteur : identifiant unique du fournisseur d’identité. Parfois appelée la « clé externe ». 
...
Configurez et mappez vos attributs d’utilisateur ou d’utilisatrice au prénom, au nom et à l’adresse e-mail correspondants.
    ...
    i. Attribut d’e-mail : indiquez le nom d’attribut que votre fournisseur d’identité utilise pour l’adresse e-mail de l’utilisateur.
    ii. Attribut du prénom : indiquez le nom d’attribut que votre fournisseur d’identité utilise pour le prénom de l’utilisateur.
    iii. Attribut de nom : indiquez le nom d’attribut que votre fournisseur d’identité utilise pour le nom de l’utilisateur.
    
    b. Conseil : si vous testez votre configuration SAML maintenant, nous analyserons les attributs E-mail, Prénom et Nom que vous pouvez utiliser pour cette section.
...
Configurez et mappez vos paramètres de rôle d’utilisateur et d’utilisatrice aux rôles ou groupes correspondants classés par votre IdP.
    ...
    a. Les clients ont la possibilité d’affecter des rôles  [!DNL Marketo Measure] ’utilisateur en fonction de groupes définis dans leur fournisseur d’identité. En saisissant vos attributs SAML,  [!DNL Marketo Measure]  sera en mesure de mapper les rôles et les groupes de vos utilisateurs et utilisatrices  [!DNL Marketo Measure]  à leurs autorisations. Nous vous recommandons vivement de configurer ces rôles afin que votre administrateur dispose  [!DNL Marketo Measure]  droits suffisants pour mettre à jour votre compte.
    
    b. Si aucun rôle ou groupe n’est mappé, le paramètre par défaut est que tous les employés du fournisseur d’identité disposeront d’un accès utilisateur standard.
    
    i. [!DNL Marketo Measure]  [!DNL Marketo Measure]  Utilisateur standard : indiquez la valeur du rôle ou du groupe (de votre fournisseur SSO) pour les utilisateurs qui doivent avoir un accès en lecture seule à l’application.
    ii. [!DNL Marketo Measure] Utilisateur administrateur du compte : indiquez la valeur du rôle ou du groupe (de votre fournisseur SSO) pour les utilisateurs qui doivent avoir un accès administratif à l’application [!DNL Marketo Measure] . Cela signifie que le rôle a accès à la modification des configurations et des paramètres liés à votre compte.
    iii. Vous devez disposer d’un attribut dans votre IdP avec le nom exact de « groupes » qui héberge les valeurs que vous avez placées dans les attributs « Utilisateur standard Bizible » ou « Utilisateur administrateur de compte Bizible ».
...
Tester la configuration de l’authentification unique
    ...
    a. Avant de pouvoir cliquer sur Enregistrer, vous devrez cliquer sur le bouton [!UICONTROL Tester l’authentification SAML] pour vérifier que vos paramètres ont été correctement configurés.
