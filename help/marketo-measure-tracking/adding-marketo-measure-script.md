---
description: Ajout  [!DNL Marketo Measure]  Script - [!DNL Marketo Measure]
...
Une bonne pratique consiste à coder en dur le code JavaScript [!DNL Marketo Measure] dans vos propriétés web. Pour coder en dur le script, vous devez le placer avant la `</head>` de fermeture sur chaque page de votre site.

`<script type="text/javascript" src="https://cdn.bizible.com/scripts/bizible.js" async=""></script>`

Le codage en dur du JavaScript dans la `<head>` de vos pages garantit que le script [!DNL Marketo Measure] se charge en premier et que les informations de référence ne sont pas manquantes. Le code JavaScript [!DNL Marketo Measure] se charge de manière asynchrone. En cas de codage en dur, le code JavaScript doit être ajouté manuellement à l’automatisation du marketing.

>[!TIP]
>Découvrez comment vous assurer que votre script est [conforme au RGPD](/help/security/ensuring-consent-for-gdpr-in-marketo-measure-js.md){target="_blank"}.

## Systèmes de gestion des balises {#tag-management-systems}

S’il n’est pas possible d’ajouter [!DNL Marketo Measure] JavaScript par codage en dur, une autre option consiste à ajouter le script [!DNL Marketo Measure] à l’aide d’un système Tag Management tel que [!DNL Google Tag Manager] (GTM) ou Tealium.

L’utilisation de systèmes de gestion des balises pour déployer [!DNL Marketo Measure] JS peut entraîner une perte de données potentielle de 5 à 10 % en raison de la latence du chargement des scripts. Essentiellement, si l’outil de gestion des balises ne se charge pas assez rapidement, le code JS [!DNL Marketo Measure] ne peut pas non plus se charger suffisamment rapidement et peut perdre les informations du premier référent.
...
Pour ajouter [!DNL Marketo Measure] script par le biais d’une solution de gestion des balises, vous devez créer une balise et y ajouter notre JavaScript. Appliquez cette balise à toutes les pages de votre site web dont vous souhaitez effectuer le suivi.

[!DNL Marketo Measure] recommande que toute page vue provoque le déclenchement de la balise. En outre, il est préférable de [!DNL Marketo Measure] donner la priorité la plus élevée dans l’ordre de déclenchement et de s’assurer qu’il n’y a aucun script synchrone devant la balise [!DNL Marketo Measure] pour garantir la qualité de données la plus élevée.

Vous trouverez de plus amples informations [ici](/help/marketo-measure-tracking/adding-marketo-measure-script-via-google-tag-manager.md){target="_blank"}.
...
Cependant, si vous utilisez des domaines distincts ou internationaux, veillez à en informer votre conseiller ou votre conseillère [!DNL Marketo Measure]. Les domaines doivent être ajoutés manuellement à votre compte du côté [!DNL Marketo Measure] afin que [!DNL Marketo Measure] sachiez lier les données des domaines supplémentaires à votre compte. Envoyez donc tous les domaines distincts/internationaux à votre consultant [!DNL Marketo Measure].

Si vous utilisez des pages tierces, discutez de votre cas d’utilisation avec votre consultant [!DNL Marketo Measure]. En règle générale, vous voudrez savoir si vous pouvez ajouter une version personnalisée de [!DNL Marketo Measure] JavaScript pour effectuer le suivi de ces pages, le cas échéant. Si cela s’avère impossible, le suivi à l’aide des points de contact CRM Campaign sera exploré avec votre consultant [!DNL Marketo Measure].
...
![Exemple d’invite d’avertissement WAF pour le script Marketo Measure](assets/adding-marketo-measure-script-1.png)

## Formulaires auxquels prêter une attention particulière {#forms-to-pay-extra-attention-to}

**Envoi de plusieurs formulaires**

* Problème : si vous avez plusieurs formulaires liés dans le cadre d’un seul envoi de formulaire, il est possible que le premier formulaire génère un point de contact même si le formulaire complet n’est pas envoyé.
* Solution : vous devez forcer l’un des formulaires à signaler l’utilisateur à [!DNL Marketo Measure] en fonction des données mises en cache et discuter des pratiques d’abandon. En général, le [code utilisateur du rapport](/help/marketo-measure-tracking/ajax-form-handling.md){target="_blank"} peut résoudre ce problème.
...
* Problème : si vos ressources sont bloquées, [!DNL Marketo Measure] effectue le suivi des téléchargements au fur et à mesure que les formulaires sont remplis. Si vos ressources ne sont pas sécurisées, les éléments pour lesquels nous pouvons créer des rapports sans personnalisation sont limités.
...
* Solution : le JS [!DNL Marketo Measure] doit être déployé directement dans l’en-tête iFrame pour que nous puissions le joindre correctement au formulaire.
...
* Solution : si vous devez savoir quel formulaire a été rempli, envisagez de configurer le hachage dynamique d’URL avec votre équipe web.
...
* Solution : ces [modèles d’utilisateurs de rapports](/help/marketo-measure-tracking/ajax-form-handling.md){target="_blank"} peuvent être utilisés par votre équipe de développement web pour ajouter le code nécessaire.
...
* Problème : [!DNL Marketo Measure] JavaScript est spécifique à un domaine. Des mesures supplémentaires doivent donc être prises pour tout domaine distinct ou international. [!DNL Marketo Measure] JS peut gérer des sous-domaines sur le même domaine racine.
...
Votre consultant [!DNL Marketo Measure] vous aidera à tester le site web de manière ponctuelle afin de vous assurer que [!DNL Marketo Measure] JavaScript est présent sur toutes les pages. Une partie de ces tests consiste à envoyer quelques remplissages de formulaire avec des détails de test clairement indiqués pour s’assurer que le suivi renvoie correctement les résultats.
...
Pour tester un formulaire vous-même, procédez comme suit :
