...
Les tableaux de bord [!DNL Marketo Measure] Discover se composent de données Buyer Touchpoint et Buyer Attribution Touchpoints, ainsi que de données CRM essentielles. La principale différence entre les rapports CRM et les rapports dans [!DNL Marketo Measure] Discover réside dans le fait que les données de point de contact dans Discover sont présentées de manière plus « agrégée » et résumées par dimension (canal marketing, campagne, etc.) par opposition aux enregistrements de points de contact individuels qui peuvent ensuite être résumés. [!DNL Marketo Measure] Discover est utilisé pour comprendre à un haut niveau quels efforts de votre part ont le plus d’impact sur les leads, les opportunités, les affaires et le montant du chiffre d’affaires qui doit leur être attribué. Une fois que le chiffre d’affaires attribué est calculé à l’aide des différents modèles d’attribution (le chemin complet est recommandé pour attribuer le chiffre d’affaires/les réservations clôturés), nous pouvons le mesurer par rapport au montant dépensé dans la même dimension (canal marketing, sous-canal ou campagne). Cela nous donne ensuite le **retour sur investissement**.
